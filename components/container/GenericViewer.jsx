...
function GenericViewer(props) {
  ...
  function handleViewModeChange() {
    console.log("Generic Viewer After change in viewmode, clean: ", currentCameraContext.current);
    console.log("Generic Viewer After change in viewmode, clean: ", currentCameraContext.current);
  ...
  const initViewer = (viewerId) => {
    switch  (viewMode) {
      case 'Design':
        console.log("Generic viewer checking existing view: ", forgeUtils.current);
        if (!forgeUtils.current) {
          ...
          console.log("iGeneric Viewer sForgeViewer Loaded: ", forge.isViewerLoaded());
    ...
    switch  (viewMode) {
      case 'Design':
        if (forgeCompareUtils.current == undefined) {
          let forge = new ForgeViewerUtils(viewerId, viewerEventHandler);
          console.log("Generic Viewer isForgeViewer Loaded: ", forge.isViewerLoaded());
  ...
  const setForgeViewerUtils = function(viewerId) {
    ...
    if(viewerId.split('_')[1] == 1) {
      console.log("Generic viewer inside main viewer: ", viewerId, viewerId.split('_')[1]);
      initViewer(viewerId);
    } else {
      console.log("Generic viewer inside compare viewer: ", viewerId, viewerId.split('_')[1]);
  ...
  function renderViewer(count) {
    ...
    switch (viewMode) {
      case "Design":
        return <ForgeViewer viewerCount={count} setForgeViewer={setForgeViewerUtils}></ForgeViewer>;
      case "Reality":
        return <PotreeViewer viewerCount={count} setPotreeViewer={setpotreeViewerUtils}></PotreeViewer>;
  ...
  const cleanUpOnViewModeChange = () => {
    console.log("Generic Viewer Inside cleanup: viewmode dependencies", viewMode);
    let camObject;
    switch (viewMode) {
      case "Design":
        if (forgeUtils.current) {
          camObject = forgeUtils.current.getCamera();
          console.log("Generic Viewer Inside clean camObject from forge: ", camObject);
          delete forgeUtils.current;
        }
        if (isCompare && forgeCompareUtils.current) delete forgeCompareUtils.current;
        break;
      case "Reality":
        if (potreeUtils.current) {
          camObject = potreeUtils.current.getCamera();
          console.log("Generic Viewer Inside clean camObject from potree: ", camObject);
          delete potreeUtils.current;
        }
        if (isCompare && potreeCompareUtils.current) delete potreeCompareUtils.current;
        break;
    }
  ...
  const cleanUpOnSnapshotChange = () => {
    console.log("Generic Viewer Inside cleanup: snapshot dependencies", snapshot);
  ...
  const cleanUpOnStructureChange = () => {
    console.log("Generic Viewer Inside cleanup structure dependencies:", currentStructure.current);
    switch (currentViewMode.current) {
      case "Design":
        if(forgeUtils.current) {
