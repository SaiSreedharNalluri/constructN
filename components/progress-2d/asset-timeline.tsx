...
const AssetTimelineContent: React.FC<{ progress: IAssetProgress, stages: IAssetStage[] }> = ({ progress, stages }) => {
    ...
    return (
        ...
        <div className='mt-[-1.5px]'>
            ...
            <Typography fontFamily='Open Sans' variant='caption' color={'#7a7a7a'}>{ moment(new Date(progress.date)).format('DD MMM, yyyy HH:mm')}</Typography>

            <Typography fontFamily='Open Sans' fontSize={13} className='mt-1 mb-4' color={'#4a4a4a'}>{ `The stage for this asset has been updated to ${_getStageFromId(progress.stage as string).name} `}</Typography>
