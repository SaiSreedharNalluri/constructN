...
export const ForgeViewerUtils = (function () {
  ...
  const getAvailableType = () => {
	...
	const initializeViewer = (viewerId, eventHandler) => {
		...
		const initializerCallBack = () => {
			...
			_viewer.navigation.setWorldUpVector(new THREE.Vector3().fromArray([0, 0, 1]), false);
  ...
  const loadLayersOnDataLoadCompletion = () => {
    console.log("Inside loadlayers On data load complete: ", _isPendingLayersToLoad, _isModelLoaded, _dataVizUtils);
  ...
  const onDataVizHandler = (event, targetObject) => {
    const result = _viewer.clientToWorld(event.originalEvent.clientX, event.originalEvent.clientY);
    switch(event.type) {
  ...
  const getContext = () => {
    ...
    if (_isModelLoaded){    
      _viewer.navigation.setCameraUpVector(new THREE.Vector3().fromArray([0, 0, 1]));
      const state = _viewer.getState({ viewport: true }).viewport;
      let viewerState = {
  ...
  const loadExtension = async() => {
    _bimWalkExtn = await _viewer.loadExtension(
      "Autodesk.BimWalk",
    );

    _dataVizExtn = await _viewer.loadExtension(
      "Autodesk.DataVisualization",
    );
  }
  ...
  const onViewerUnInitialized = () => {
	...
	const setUpEventListeners = () => {
		_viewer.addEventListener( Autodesk.Viewing.VIEWER_INITIALIZED, onViewerInitialized);
		_viewer.addEventListener( Autodesk.Viewing.VIEWER_UNINITIALIZED, onViewerUnInitialized);
		_viewer.addEventListener( Autodesk.Viewing.PROGRESS_UPDATE_EVENT, modelLoadProgress);
		_viewer.addEventListener( Autodesk.Viewing.MODEL_LAYERS_LOADED_EVENT, onModelLayersLoadedEvent);
		_viewer.addEventListener( Autodesk.Viewing.GEOMETRY_LOADED_EVENT, onGeometryLoadedEvent);
		_viewer.addEventListener( Autodesk.Viewing.MODEL_UNLOADED_EVENT, onModelUnLoadedEvent);
		_viewer.addEventListener( Autodesk.Viewing.EXTENSION_LOADED_EVENT, onExtensionLoadedEvent);
		_viewer.addEventListener( Autodesk.Viewing.CAMERA_CHANGE_EVENT, onCameraChangeEvent);
	...
	const removeEventListeners = () => {
		_viewer.removeEventListener( Autodesk.Viewing.VIEWER_INITIALIZED, onViewerInitialized);
		_viewer.removeEventListener( Autodesk.Viewing.VIEWER_UNINITIALIZED, onViewerUnInitialized);
		_viewer.removeEventListener(Autodesk.Viewing.PROGRESS_UPDATE_EVENT, modelLoadProgress);
		_viewer.removeEventListener( Autodesk.Viewing.MODEL_LAYERS_LOADED_EVENT, onModelLayersLoadedEvent);
		_viewer.removeEventListener( Autodesk.Viewing.GEOMETRY_LOADED_EVENT, onGeometryLoadedEvent);
		_viewer.removeEventListener( Autodesk.Viewing.MODEL_UNLOADED_EVENT, onModelUnLoadedEvent);
		_viewer.removeEventListener( Autodesk.Viewing.EXTENSION_LOADED_EVENT, onExtensionLoadedEvent);
		_viewer.removeEventListener( Autodesk.Viewing.CAMERA_CHANGE_EVENT, onCameraChangeEvent);
