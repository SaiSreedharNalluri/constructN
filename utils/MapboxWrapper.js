...
export const MapboxViewerUtils = () => {
  ...
  const onLayerClick = (e, hidePopup) => {
    ...
    let coordinates =
      e.geometry.type === "Polygon"
        ? getCentreOfFeatureCollection(e)
    ...
    if (!hidePopup) {
      if (!_popup) {
        _popup = new mapboxgl.Popup({ closeOnClick: true })
          ...
          .setHTML(
            e.properties.htmlInfo ? e.properties.htmlInfo : getHtmlInfo(e)
          )
          .addTo(_map);
      } else {
        _popup
  ...
  const getHtmlInfo = (e) => {
    ...
    html +=
      '<div style="font-weight:600;border-bottom: 1px solid #aaa;margin-bottom: 0.5em;">' +
      e.properties.name +
      "</div>";
    html +=
      '<div style="display:flex; padding:0 0.5em; background:rgba(0,255,200,0.5)"><div style="flex: 1";>' +
      "Progress" +
      ': </div><strong style="margin-left: 1em;">' +
      progress.toFixed(0) +
      "%</strong></div>";
    html +=
      '<div style="display:flex; padding:0 0.5em;"><div style="flex: 1";>' +
      "Total length" +
      ': </div><strong style="margin-left: 1em;">' +
      e.properties.length.toFixed(2) +
      " mts.</strong></div>";
    ...
    html +=
      ...
      "Row Clearing" +
      ': </div><strong style="margin-left: 1em;">' +
      e.properties.row_clearing.toFixed(2) +
      " mts.</strong></div>";
    html +=
      '<div style="display:flex; padding:0 0.5em;"><div style="flex: 1";>' +
      "Stringing" +
    ...
    html +=
      ...
      "Excavation" +
      ': </div><strong style="margin-left: 1em;">' +
      e.properties.excavation.toFixed(2) +
      " mts.</strong></div>";
    html +=
      '<div style="display:flex; padding:0 0.5em;"><div style="flex: 1";>' +
      "Pipe Laying" +
      ': </div><strong style="margin-left: 1em;">' +
      e.properties.pipe_laying.toFixed(2) +
      " mts.</strong></div>";
    html +=
      '<div style="display:flex; padding:0 0.5em;"><div style="flex: 1";>' +
      "OFC Cabling" +
      ': </div><strong style="margin-left: 1em;">' +
      ofc_cabling.toFixed(2) +
      " mts.</strong></div>";
    html +=
      '<div style="display:flex; padding:0 0.5em;"><div style="flex: 1";>' +
      "Mat Laying" +
      ': </div><strong style="margin-left: 1em;">' +
      mat_laying.toFixed(2) +
      " mts.</strong></div>";
    html +=
      '<div style="display:flex; padding:0 0.5em;"><div style="flex: 1";>' +
      "Back Filling" +
      ': </div><strong style="margin-left: 1em;">' +
      e.properties.back_filling.toFixed(2) +
      " mts.</strong></div>";
