...
const Support: React.FC<IProps> = ({  }) => {
 ...
 if(eMail!=="")
  ...
  const getMyTickets =()=>{
   // console.log(eMail);
    return  axios.get(`https://constructnai.freshdesk.com/api/v2/tickets?email=${eMail}`, {headers:fresh_headers})
    .then((response) => {
  ...
  function openChat(): void {
    ...
    //{eval(`globalThis.fcWidget.user.setFirstName("John");`)}
  ...
  return (
     ...
     <div className=' self-center place-content-center flex-auto  w-2/3 overflow-auto'>
      ...
      <Formik
            ...
            onSubmit={(values,action)=>{
              ...
              values.email=eMail;
              axios.post('https://constructnai.freshdesk.com/api/v2/tickets', {
      'email':values.email,'subject':values.subject,'status':values.status,'priority':values.priority,'description':values.description},{headers:fresh_headers}
    )
    .then((response) => {
    ...
    .catch((error) => {
      ...
      throw error;
          ...
          <div>
          <div id="custom_fc_button" className='absolute bg-orange-400 rounded-full left-0 bottom-4 w-4' onClick={openChat}>
          <button className='p-1'  disabled={false} >Chat</button>
          </div>
          </div>
          <div className='p-5 align-middle'>
            ...
            return(<li key={ticket.id}>
              <div className='flex flex-col'>
                <div><div className='text-xl flex font-bold'>#{ticket.id}</div><div className='text-xl'>{ticket.subject}</div>
                </div> 
